<rule_activation id="a9b236a8-a06c-402b-a958-063272e958eb" title="Adopt Message Queue Integration Pattern for External System Communication" applies_to="**/*">
These rules are ALWAYS ACTIVE for all files. External API integrations for browser detection, test coverage, system monitoring, and other external services must use message queue boundaries for communication.
</rule_activation>
...
```bash
# Check for message queue boundary usage
...
# Find external API integrations using message queue patterns
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Automated code review checks scan for direct external API calls without message queue boundaries. CI/CD pipeline fails if direct synchronous external API calls are detected without approved exceptions. Architecture review required for new external API integrations.
</enforcement>