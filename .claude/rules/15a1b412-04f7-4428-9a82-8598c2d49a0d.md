<rule_activation id="15a1b412-04f7-4428-9a82-8598c2d49a0d" title="Standardize Cache Layer Implementation for Module Resolution and State Management" applies_to="**/*">
These rules are ALWAYS ACTIVE for all files in the codebase.

Applies to all library and module code that implements caching mechanisms for data, computation results, or API responses, including:
...
```bash
# Check for cache implementations in key modules
...
- Automated linting rules detect non-standard cache implementations during CI builds
- Code review checklist includes verification of cache layer compliance
- Monthly architecture review of new cache implementations
- Static analysis tools scan for direct Map/WeakMap usage in caching contexts

Violation handling:
- CI build fails if non-standard cache implementations are detected without documented exceptions
- Pull requests with cache-related changes require approval from a designated cache architecture owner
- Violations discovered post-merge are tracked as technical debt items with priority based on impact
- Quarterly audits identify and prioritize remediation of non-compliant cache implementations

Exception process:
- Submit exception request to Tech Lead or Staff Engineer with performance justification and benchmark data
- Document exception in code comments with ADR reference and approval details
- Add exception to centralized tracking document with review date (6-12 months)
- Re-evaluate exceptions during major version upgrades or architecture reviews
</enforcement>