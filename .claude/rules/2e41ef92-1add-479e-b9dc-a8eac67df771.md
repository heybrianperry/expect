<rule_activation id="2e41ef92-1add-479e-b9dc-a8eac67df771" title="Standardize Environment Variable Validation and Path Resolution for Runtime Configuration" applies_to="**/*">
...
# Check for path joins with env vars without normalization
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All environment variable access and path resolution MUST be validated according to these rules. Pull requests with unvalidated environment variable access are blocked until validation is added.
</enforcement>