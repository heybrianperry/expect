<rule_activation id="e577ef9d-2dcb-45a7-b192-31d673f91f30" title="Adopt Browser-Based Rendering Model with Session Replay Capabilities" applies_to="**/*">
...
- **R-BR-001** MUST: All frontend rendering MUST be performed in a browser environment with full DOM access to enable accurate event capture and replay.
- **R-BR-002** MUST: Session replay components MUST capture sufficient state information to reconstruct user interactions with high fidelity.
- **R-BR-003** MUST: Locator resolution utilities MUST provide consistent element identification across live and replay contexts.
- **R-BR-004** SHOULD: Recording and replay functionality SHOULD be implemented as separate, composable modules to support different use cases (debugging, demos, testing).
- **R-BR-005** SHOULD: The rendering model SHOULD support video output generation from recorded sessions for documentation and demonstration purposes.
- **R-BR-006** MAY: Implementations MAY optimize replay performance by pre-processing recorded events or using virtual DOM techniques.
...
# Check for resolve-locator utilities usage
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All rendering implementations must comply with browser-based rendering and session replay requirements. Violations require architecture review board approval.
</enforcement>