<rule_activation id="0f80a1a7-7f62-47d0-9767-18b08d765aa1" title="Standardize Structured Logging with Public API Contracts" applies_to="**/*">
...
- **R-LOG-006** MUST NOT: Logger implementations MUST NOT directly couple to specific logging backends (e.g., console, file, external services) in their public API contracts.
...
- Include examples showing structured logging with context objects: `logger.info('User action', { userId, action, timestamp })`
...
<enforcement>
Verification is mandatory. Claude Code MUST run the verify commands and check acceptance criteria before considering logging implementations complete. CI pipeline fails on detection of direct console.log/console.error usage in production code. Code review blocks merge requests that introduce non-compliant logging patterns.

Exception process:
1. Developer submits exception request with justification (performance data, legacy code risk assessment, etc.)
2. Engineering lead or architecture review board evaluates request against documented exception criteria
3. Approved exceptions must be documented in code with comments explaining rationale and linking to approval
4. Exception registry maintained with periodic review to identify opportunities for remediation
</enforcement>