<rule_activation id="ff41567f-1f8f-4750-b0f7-ede483530b2b" title="Standardize Structured Logging with Public API Contracts" applies_to="**/*">
...
<enforcement>
Claude Code MUST verify logging implementations against these rules. Automated linting rules in CI pipeline check for direct console usage and enforce structured logging patterns. Code review checklist includes verification of logging contract compliance. Violations result in CI pipeline failures and blocked merge requests. Exception requests must be submitted with performance data or risk assessment and approved by engineering lead or architecture review board.
</enforcement>